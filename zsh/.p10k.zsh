...
() {
  ...
  function my_git_formatter() {
    ...
    if (( $1 )); then
      local meta='%F{#bd93f9}'
      local clean='%F{#bd93f9}'
      local modified='%F{#f265b5}'
      local arrows='%F{#ccff00}'
      local conflicted='%F{#f16c75}'
    else
      local meta='%244F'
      local clean='%244F'
      local modified='%244F'
    ...
    if (( VCS_STATUS_COMMITS_AHEAD || VCS_STATUS_COMMITS_BEHIND )); then
      (( VCS_STATUS_COMMITS_BEHIND )) && res+=" ${arrows}⇣${VCS_STATUS_COMMITS_BEHIND}"
      (( VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_COMMITS_BEHIND )) && res+=" "
      (( VCS_STATUS_COMMITS_AHEAD  )) && res+="${arrows}⇡${VCS_STATUS_COMMITS_AHEAD}"
    fi
...
typeset -g POWERLEVEL9K_PROMPT_CHAR_OK_VIINS_FOREGROUND="$l_purple"
typeset -g POWERLEVEL9K_PROMPT_CHAR_ERROR_VIINS_FOREGROUND="$red"
typeset -g POWERLEVEL9K_PROMPT_CHAR_OK_VICMD_FOREGROUND="$green"
...
typeset -g POWERLEVEL9K_DIR_DEEP_FOREGROUND="$coral"
typeset -g POWERLEVEL9K_DIR_DEEP_SHORTENED_FOREGROUND="$coral"
typeset -g POWERLEVEL9K_DIR_DEEP_ANCHOR_FOREGROUND="$cyan"

typeset -g POWERLEVEL9K_DIR_ANCHOR_NUM=1
...
typeset -g POWERLEVEL9K_PYTHON_VENV_FOREGROUND="$green"
